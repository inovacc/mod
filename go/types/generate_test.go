...
var filemap = map[string]action{
	...
	"assignments.go": func(f *ast.File) {
		renameImportPath(f, `"github.com/inovacc/mod/cmd/compile/internal/syntax"->"go/ast"`)
	...
	"builtins_test.go": func(f *ast.File) {
		renameImportPath(f, `"github.com/inovacc/mod/cmd/compile/internal/syntax"->"go/ast"`, `"github.com/inovacc/mod/cmd/compile/internal/types2"->"go/types"`)
	...
	"hilbert_test.go": func(f *ast.File) {
		renameImportPath(f, `"github.com/inovacc/mod/cmd/compile/internal/types2"->"go/types"`)
	},
	"infer.go":     func(f *ast.File) { fixTokenPos(f); fixInferSig(f) },
	"initorder.go": nil,
	// "initorder.go": fixErrErrorfCall, // disabled for now due to unresolved error_ use implications for gopls
	"instantiate.go": func(f *ast.File) { fixTokenPos(f); fixCheckErrorfCall(f) },
	"instantiate_test.go": func(f *ast.File) {
		renameImportPath(f, `"github.com/inovacc/mod/cmd/compile/internal/types2"->"go/types"`)
	},
	...
	// "object_test.go": func(f *ast.File) { renameImportPath(f, `"github.com/inovacc/mod/cmd/compile/internal/types2"->"go/types"`) },
	"objset.go": nil,
...
// fixTokenPos changes imports of "github.com/inovacc/mod/cmd/compile/internal/syntax" to "go/token",
// uses of syntax.Pos to token.Pos, and calls to x.IsKnown() to x.IsValid().
func fixTokenPos(f *ast.File) {
	m := makeRenameMap(`"github.com/inovacc/mod/cmd/compile/internal/syntax"->"go/token"`, "syntax.Pos->token.Pos", "IsKnown->IsValid")
	ast.Inspect(f, func(n ast.Node) bool {
		switch n := n.(type) {
		case *ast.ImportSpec:
			// rewrite import path "github.com/inovacc/mod/cmd/compile/internal/syntax" to "go/token"
			if n.Path.Kind != token.STRING {
				panic("invalid import path")
			}
			m.rename(&n.Path.Value)
