...
// crash 3
type t3 *interface {
	t3.p /* ERROR "t3.p is not a type" */
}

// crash 4
type Numeric4 interface {
	t4 /* ERROR "not a type" */
}

func t4[T Numeric4](s []T) {
	if 0 {
		*s /* ERROR "cannot indirect" */ [0]
	}
}
...
// crash 8
type foo8[A any] interface {
	~A /* ERROR "cannot be a type parameter" */
}
...
func F20[t Z20]() {
	F20(t /* ERROR "invalid composite literal type" */ /* ERROR "too many arguments in call to F20\n\thave (unknown type)\n\twant ()" */ {})
}
...
// crash 26
type T26 = interface {
	F26[ /* ERROR "interface method must have no type parameters" */ Z any]()
}

func F26[Z any]() T26 { return F26[] /* ERROR "operand" */ }

// crash 27
func e27[T any]() interface {
	x27 /* ERROR "not a type" */
} {
	panic(0)
}
